...
class Collection(element.Element):
    ...
    def __init__(self, args=None, version=None):
        ...
        if isinstance(args, computedobject.ComputedObject):
            ...
            super(Collection, self).__init__(args.func, args.args, args.varName)
        elif isinstance(args, dict):
            # 如果是字典类型
            keys_to_extract = set(['baseUrl', 'collectionId'])
            request = {}
            collection_params = {}
            if args:
                for key in args:
                    if key in keys_to_extract:
                        collection_params[key] = args[key]
                    else:
                        request[key] = args[key]
            super(Collection, self).__init__(
                apifunction.ApiFunction.lookup('Service.getCollection'), collection_params)
    ...
    """
        if not new_filter:
            raise oge_exception.OGException('Empty filters.')
