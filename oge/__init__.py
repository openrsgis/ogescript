...
from .oge_exception import OGException
from .oge_list import List
from .oge_number import Number
from .oge_string import String
  ...
  """
    ...
    if klass == 'Image':
        ...
        if isinstance(arg, Element):
            ...
            # No way to convert.
            raise OGException('Cannot convert {0} to Element.'.format(arg))
    elif klass == 'Geometry':
        if isinstance(arg, Collection):
            return ApiFunction.call_('Collection.geometry', arg)
        else:
            return Geometry(arg)
    elif klass == 'Collection':
    ...
    elif klass == 'CoverageCollection':
        return CoverageCollection(arg)
    elif klass == 'Filter':
        return Filter(arg)
        ...
        if isinstance(arg, six.string_types):
            ...
            raise OGException('Argument is not a function: {0}'.format(arg))
        ...
        if isinstance(arg, cls):
            ...
            if hasattr(cls, arg):
                ...
                raise OGException('Unknown algorithm: {0}.{1}'.format(klass, arg))
        else:
            # Client-side cast.
            return cls(arg)
    else:
