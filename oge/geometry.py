...
class Geometry(computedobject.ComputedObject):
    ...
    """
        ...
        if not Geometry._isValidGeometry(geo_json):
            raise oge_exception.OGException('Invalid GeoJSON geometry.')
        ...
        if opt_proj:
            ...
            if (isinstance(geo_json.get('crs'), dict) and
                ...
                raise oge_exception.OGException('Invalid CRS declaration in GeoJSON: ' +
    ...
    """
        ...
        if all(oge_types.isNumber(i) for i in args):
    ...
    Raises:
      OGException: if the depth is invalid and could not be fixed.
    """
        if depth < 1 or depth > 4:
            raise oge_exception.OGException('Unexpected nesting level.')
        ...
        if Geometry._isValidCoordinates(coords) != depth:
            raise oge_exception.OGException('Invalid geometry.')
