...
class MapClientObject():
    def __init__(self, lng, lat, zoom):
        self.lng = lng
        self.lat = lat
        self.zoom = zoom
...
def addToMap(eeobject, vis_params=None, *unused_args):
  ...
  """
    ...
    dagDict = eeobject.getMapId(vis_params)["values"]
    dagJson = json.dumps(dagDict)
    # http_util.HTTPUtil.postDagJson(dagJson)
    print(dagJson)
    # mapClient.set_center(108.90494046724021, 18.753457222586285)
    # res = HTTPUtil.postRequest(url="http://125.220.153.26:8085/oge-dag/postjsonstring", param = DAGJson)
    # async def get_data():
    #     response = await pyfetch('http://125.220.153.26:8085/oge-dag/saveDagJson', method="POST", body=dagJson)
...
#     res = {"vector":[{"url":"http://125.220.153.26:8093/ogedemooutput/oge_flooded_1666165669048.geojson"}],"raster":[{"url":"http://125.220.153.26:8093/ogedemooutput/oge_flooded1666165669048.png","extent":[18.0,108.0,20.0,110.0]}]}
#     param={"Dag": DAGJson})
# res = eval(res1)
# res = {"vector": [], "raster": [{"url": "http://125.220.153.26:8093/lgedemooutput/lge_1659511761732.png",
#                                  "extent": [29.53034037272259, 113.04278793924837, 30.819695204669433,
#                                             114.35392984936713]}]}
# res = {"raster": [
#     {"url": "http://125.220.153.26:8093/lgedemooutput/flooded.png", "extent": [18.0, 108.0, 20.0, 110.0]}],
#        "vector": []}
#     firstExtent = res["raster"][0]["extent"]
#     a = [(firstExtent[0] + firstExtent[2]) / 2.0,(firstExtent[1] + firstExtent[3]) / 2.0]
#     mapClient.center = a
#     # mapClient.center([(firstExtent[1] + firstExtent[3]) / 2.0, (firstExtent[0] + firstExtent[2]) / 2.0])
#     vectorList = res["vector"]
#     rasterList = res["raster"]
#     for vector in vectorList:
#         url = vector["url"]
#         r = requests.get(url)
#         with open('../json_file/flooded.geojson', 'w') as f:
#             f.write(r.content.decode("utf-8"))
#         data = json.load(open("../json_file/flooded.geojson", 'r'))
#         geo_json = GeoJSON(data=data)
#         geo_json.on_click(klick)
#         mapClient.add_layer(geo_json)
...
#         )
#         mapClient.add_layer(image)
#     marker = Marker(location=(52.204793, 360.121558), draggable=False)
#     mapClient.add_layer(marker)
#     return mapClient
# res = {"urlList": ["http://125.220.153.26:8093/lgedemooutput/flooded.png"], "bounds": [[18, 108, 20, 110]]}
# # TODO 这里res是请求的返回值， 包含两个部分 res.urlList=[url1,url2,url3.....] res.bounds=[[SW_lat, SW_lng, NE_lat, NE_lng],
# #  ......]  形式可自行更改 ；如果要叠加WMS瓦片，参考官网https://ipyleaflet.readthedocs.io/en/latest/layers/wms_layer.html
# # mapClient.add_point_layer("http://125.220.153.26:8093/lgedemooutput/flooded.geojson", popup="name", layer_name="US Cities")
# data = json.load(open("E:\\LaoK\\code_test\\lge_python\\json_file\\flooded.geojson", 'r', encoding="utf-8"))
# geo_json = GeoJSON(
#     data = data)
# mapClient.add_layer(geo_json)
# for i in range(0, len(res["urlList"])):
#     image = ImageOverlay(
#         url=res["urlList"][i],
#         bounds=((res["bounds"][i][0], res["bounds"][i][1]), (res["bounds"][i][2], res["bounds"][i][3]))
#     )
#     mapClient.add_layer(image)
# return mapClient
...
def centerMap(lng, lat, zoom):  # pylint: disable=g-bad-name
    ...
    print("spaceParams=<<" + str(space_params) + ">>")
    global map_instance
    if not map_instance:
        map_instance = MapClientObject(lng, lat, zoom)
    else:
        map_instance = MapClientObject(lng, lat, zoom)
...
def getSpaceParams():
    if not map_instance:
        return None
    return map_instance.getSpaceParams()
