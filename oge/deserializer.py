...
def _decodeValue(json_obj, named_values):
  ...
  """
    ...
    if not isinstance(json_obj, dict):
        raise oge_exception.OGException('Cannot decode object: ' + json_obj)
    ...
    if type_name == 'ValueRef':
        if json_obj['value'] in named_values:
            ...
            raise oge_exception.OGException('Unknown ValueRef: ' + json_obj)
    elif type_name == 'ArgumentRef':
        var_name = json_obj['value']
        if not isinstance(var_name, six.string_types):
            raise oge_exception.OGException('Invalid variable name: ' + var_name)
        ...
        if not isinstance(microseconds, numbers.Number):
            raise oge_exception.OGException('Invalid date value: ' + microseconds)
        return oge_date.Date(microseconds / 1e3)
        ...
        raise oge_exception.OGException('Nested CompoundValues are disallowed.')
    else:
        raise oge_exception.OGException('Unknown encoded object type: ' + type_name)
...
def _invocation(func, args):
    ...
    raise oge_exception.OGException('Invalid function value: %s' % func)
    ...
    def lookup(reference, kind):
        if reference not in decoded:
            if reference not in json_obj['values']:
                raise oge_exception.OGException('Cannot find %s %s' % (reference, kind))
